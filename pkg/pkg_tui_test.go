...
func TestOptionRowsMarkTheCursor(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			m := pkg.NewNotifyModel(pkg.Notification{
				Options:  []string{"yes", "no"},
				Multiple: test.multiple,
				Cursor:   pkg.NotifyCursor{Single: ">", Multiple: "+"},
			})
...
func TestOptionRowsLineUpUnderAnyMarker(t *testing.T) {
	for _, mark := range []string{"", ">", "❯", "🔥"} {
		m := pkg.NewNotifyModel(pkg.Notification{
			Options: []string{"yes", "no"},
			Cursor:  pkg.NotifyCursor{Single: mark},
		})
...
func TestOptionRowsWithoutAMarker(t *testing.T) {
	m := pkg.NewNotifyModel(pkg.Notification{Options: []string{"yes", "no"}})
